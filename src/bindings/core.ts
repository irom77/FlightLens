...
export type SummaryRequest = { "kind": "inspector", configId: string, rateProfile: number, pidProfile: number, } | { "kind": "diff", slots: Array<SummarySlot>, baseline: number | null, rows: Array<DiffRowInput>, };

export type SummarySlot = { configId: string, rateProfile: number, pidProfile: number, };

export type LlmStatus = { settings: LlmSettings, hasKey: boolean, keyHint: string | null, sessionOnly: boolean, credentialProblem: string | null, };

export type LlmPreview = { systemPrompt: string, userPrompt: string, payloadJson: string, excluded: Array<Redaction>, labelMap: Array<[string, string]>, bytes: number, truncated: Truncation | null, problems: Array<string>, destination: string, };

export type LlmSummary = { text: string, provider: Provider, model: string, promptVersion: number, generatedAt: string, cached: boolean, };

export type Provider = "gemini" | "openai" | "openrouter" | "custom";

export type LlmSettings = { enabled: boolean, provider: Provider, model: string, baseUrl: string | null, timeoutSeconds: number, };

export type DigestFirmware = { family: string, version: string | null, packId: string | null, hasDumpAll: boolean, };

export type Provenance = "declared" | "derived" | "unknown";

export type DigestParameter = { semanticKey: string, value: string | null, unit: string | null, scope: Scope, provenance: Provenance, supported: boolean, };

export type DigestCurve = { name: string, available: boolean, samples: Array<Point>, reason: string | null, derivedInputs: Array<string>, };

export type DigestAudit = { id: string, status: string, severity: string, explanation: string, };

export type InspectorDigest = { label: string, firmware: DigestFirmware, rateProfile: number, pidProfile: number, parameters: Array<DigestParameter>, rates: Array<DigestCurve>, throttle: DigestCurve, audits: Array<DigestAudit>, diagnostics: { [key in string]?: number }, };

export type DiffRowInput = { section: string, key: string, scope: string, status: string, values: Array<string | null>, reason: string | null, };

export type Truncation = { omitted: number, sections: Array<string>, };

export type DiffDigest = { labels: Array<string>, baseline: number | null, rows: Array<DiffRowInput>, truncated: Truncation | null, };

export type WorkspaceEntry = { id: string, relativePath: string, bytes: number, modifiedSeconds: number | null, };

export type WorkspacePage = { root: string, status: string, visited: number, skipped: number, indexed: number, total: number, offset: number, entries: Array<WorkspaceEntry>, };
