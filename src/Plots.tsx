...
export function Plot({
  ...
  const min = throttle ? 0 : frequency ? -60 : -max;
  const xmax = throttle
    ? 100
    : frequency
      ? Math.max(1, ...valid.flatMap((c) => c.points.map((p) => p.x)))
      : 1;
  const xmin = frequency || throttle ? 0 : -1;
  ...
  return (
    <div className="plot-wrap">
      <svg
        ...
        aria-label={
          throttle
            ? "Throttle curve: normalized throttle input (%) versus throttle command (%)"
            : frequency
        ...
        {valid.map((c) => {
          ...
          return (
            <span key={c.name} className={`series-${c.seriesIndex % 3}`}>
              {c.name}{" "}
              {point
                ? `${point.y.toFixed(1)} ${throttle ? "%" : frequency ? "dB" : "°/s"}`
                : ""}
            </span>
          );
        })}
