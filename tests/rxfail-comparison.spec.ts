...
const fixtures = [["--rxfail-baseline"], ["--rxfail-differences"]].map(
  (flags) =>
    JSON.parse(
      execFileSync(
...
test("receiver failsafe comparison normalizes parser declarations", async ({
  page,
}) => {
  ...
  const parameters = page.getByRole("region", {
    name: "Receiver failsafe comparison",
    exact: true,
  });
  await parameters.getByLabel("Hide equal receiver failsafes").uncheck();
  for (const [key, status] of [
    ...
    await parameters.getByLabel("Filter compared receiver channels").fill(key);
    const row = parameters
      .getByRole("row")
      .filter({ has: page.getByRole("cell", { name: key, exact: true }) });
    await expect(row).toContainText(status);
    if (key === "0") {
      await expect(row).toContainText("Stored value: 1000 µs");
      await row.getByText(/B: Line/).click();
      await expect(row.locator("pre").last()).toHaveText("rxfail 0 s 1024");
    }
  }
});
